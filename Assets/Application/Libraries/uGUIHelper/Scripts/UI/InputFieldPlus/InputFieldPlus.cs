...
namespace UnityEngine.UI
{
		...
		ILayoutElement
	{
		...
		public enum CharacterValidationTypes
		{
			None,
			Integer,
			Decimal,
		...
		[SerializeField]
		private ContentTypes m_ContentType = ContentTypes.Standard ;
		...
		[SerializeField]
		private InputTypes m_InputType = InputTypes.Standard;
		...
		[SerializeField]
		private LineTypes m_LineType = LineTypes.SingleLine;
		...
		[SerializeField]
		private CharacterValidationTypes m_CharacterValidation = CharacterValidationTypes.None;
...
//					case RuntimePlatform.TizenPlayer :
		...
		public string text
		{
			...
			set
			{
				...
				if( m_LineType == LineTypes.SingleLine )
				{
					value = value.Replace( "\n", "" ).Replace( "\t", "" ) ;
				}

				// If we have an input validator, validate the input and apply the character limit at the same time.
				if( onValidateInput != null || characterValidation != CharacterValidationTypes.None )
				{
					m_Text = "" ;
		...
		public ContentTypes contentType { get { return m_ContentType; } set { if ( SetStruct(ref m_ContentType, value)) EnforceContentType(); } }
		...
		public LineTypes lineType
		{
			...
			set
			{
				...
				{
					SetToCustomIfContentTypeIsNot(ContentTypes.Standard, ContentTypes.Autocorrected);
					EnforceTextHOverflow();
				}
			}
		}

		public InputTypes inputType { get { return m_InputType; } set { if ( SetStruct(ref m_InputType, value)) SetToCustom(); } }
		...
		public CharacterValidationTypes characterValidation { get { return m_CharacterValidation; } set { if ( SetStruct(ref m_CharacterValidation, value)) SetToCustom(); } }

		public bool readOnly { get { return m_ReadOnly; } set { m_ReadOnly = value; } }

		// Derived property
		public bool multiLine { get { return m_LineType == LineTypes.MultiLineNewline || lineType == LineTypes.MultiLineSubmit; } }
			...
			if( m_Text != val )
			{
				...
				else
				{
					...
					for( int i  = 0 ; i <  val.Length ; ++ i )
					{
						...
						if( characterValidation != CharacterValidationTypes.None )
						{
							c = Validate( m_Text, m_Text.Length, c ) ;
						}

						if( lineType == LineTypes.MultiLineSubmit && c == '\n' )
						{
							m_Keyboard.text = m_Text ;
...
//				if( m_Keyboard.wasCanceled )
		...
		protected EditState KeyPressed( Event evt )
		{
			...
			switch( evt.keyCode )
			{
				...
				{
					if( lineType != LineTypes.MultiLineNewline )
					{
...
//			return m_TextComponent.font.HasCharacter( c ) ;
		...
		protected char Validate(string text, int pos, char ch)
		{
			...
			if (characterValidation == CharacterValidationTypes.None || !enabled)
				return ch;

			if (characterValidation == CharacterValidationTypes.Integer || characterValidation == CharacterValidationTypes.Decimal)
			{
				// Integer and decimal
				...
				if (!cursorBeforeDash || dashInSelection)
				{
					...
					if (ch == '.' && characterValidation == CharacterValidationTypes.Decimal && !text.Contains(".")) return ch;
				}
			}
			else if (characterValidation == CharacterValidationTypes.Alphanumeric)
			{
			...
			else if (characterValidation == CharacterValidationTypes.Name)
			{
			...
			else if (characterValidation == CharacterValidationTypes.EmailAddress)
			{
		...
		private void ActivateInputFieldInternal()
		{
			...
			if (TouchScreenKeyboard.isSupported)
			{
				...
				m_Keyboard = (inputType == InputTypes.Password) ?
					TouchScreenKeyboard.Open(m_Text, keyboardType, false, multiLine, true) :
					TouchScreenKeyboard.Open(m_Text, keyboardType, inputType == InputTypes.AutoCorrect, multiLine);
		...
		private void EnforceContentType()
		{
			switch (contentType)
			{
				case ContentTypes.Standard:
				{
					...
					m_InputType = InputTypes.Standard;
					m_KeyboardType = TouchScreenKeyboardType.Default;
					m_CharacterValidation = CharacterValidationTypes.None;
					break;
				}
				case ContentTypes.Autocorrected:
				{
					// Don't enforce line type for this content type.
					m_InputType = InputTypes.AutoCorrect;
					m_KeyboardType = TouchScreenKeyboardType.Default;
					m_CharacterValidation = CharacterValidationTypes.None;
					break;
				}
				case ContentTypes.IntegerNumber:
				{
					m_LineType = LineTypes.SingleLine;
					m_InputType = InputTypes.Standard;
					m_KeyboardType = TouchScreenKeyboardType.NumberPad;
					m_CharacterValidation = CharacterValidationTypes.Integer;
					break;
				}
				case ContentTypes.DecimalNumber:
				{
					m_LineType = LineTypes.SingleLine;
					m_InputType = InputTypes.Standard;
					m_KeyboardType = TouchScreenKeyboardType.NumbersAndPunctuation;
					m_CharacterValidation = CharacterValidationTypes.Decimal;
					break;
				}
				case ContentTypes.Alphanumeric:
				{
					...
					m_CharacterValidation = CharacterValidationTypes.Alphanumeric;
					break;
				}
				case ContentTypes.Name:
				{
					m_LineType = LineTypes.SingleLine;
					m_InputType = InputTypes.Standard;
					m_KeyboardType = TouchScreenKeyboardType.NamePhonePad;
					m_CharacterValidation = CharacterValidationTypes.Name;
					break;
				}
				case ContentTypes.EmailAddress:
				{
					m_LineType = LineTypes.SingleLine;
					m_InputType = InputTypes.Standard;
					m_KeyboardType = TouchScreenKeyboardType.EmailAddress;
					m_CharacterValidation = CharacterValidationTypes.EmailAddress;
					break;
				}
				case ContentTypes.Password:
				{
					m_LineType = LineTypes.SingleLine;
					m_InputType = InputTypes.Password;
					m_KeyboardType = TouchScreenKeyboardType.Default;
					m_CharacterValidation = CharacterValidationTypes.None;
					break;
				}
				case ContentTypes.Pin:
				{
					m_LineType = LineTypes.SingleLine;
					m_InputType = InputTypes.Password;
					m_KeyboardType = TouchScreenKeyboardType.NumberPad;
		...
		void SetToCustomIfContentTypeIsNot(params ContentTypes[] allowedContentTypes)
		{
			if (contentType == ContentTypes.Custom)
				return;
