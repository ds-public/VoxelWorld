...
namespace uGUIHelper
{
	...
	public class GridMap : MaskableGraphicWrapper
	{
		...
		[SerializeField][HideInInspector]
		private TransitionType m_TransitionType = TransitionType.None ;
					...
					for( iy  = 0 ; iy <= 2 ; iy ++ )
					{
						for( ix  = 0 ; ix <= 2 ; ix ++ )
						{
							if( ( m_VertexDensity == VertexDensity.Low && ( ci == 0 || ci == 2 || ci == 4 || ci == 6 || ci == 8 ) ) || ( m_VertexDensity == VertexDensity.High ) )
							{
								...
								if( m_TransitionType == TransitionType.CircleInToOut || m_TransitionType == TransitionType.CircleOutToIn )
								{
									// 中心から１が広がっていく(円)
									...
									if( m_TransitionType == TransitionType.CircleOutToIn )
									{
										pv = 1.5f - pv ;
									}
								...
								if( m_TransitionType == TransitionType.RectangleInToOut || m_TransitionType == TransitionType.RectangleOutToIn )
								{
									// 中心から１が広がっていく(四角)
									float px =     ( vx - cx ) / hx   ;	// -1 ～ 0 ～ +1
									float py = - ( ( vy - cy ) / hy ) ;	// -1 ～ 0 ～ +1
									...
									if( m_TransitionType == TransitionType.RectangleOutToIn )
									{
										pv = 1.5f - pv ;
									}
								...
								if( m_TransitionType == TransitionType.LeftToRight || m_TransitionType == TransitionType.RightToLeft  || m_TransitionType == TransitionType.TopToBottom || m_TransitionType == TransitionType.BottomToTop )
								{
									// 左右上下
									float px =     ( vx - cx ) / hx   ;	// -1 ～ 0 ～ +1
									float py = - ( ( vy - cy ) / hy ) ;	// -1 ～ 0 ～ +1
									...
									if( m_TransitionType == TransitionType.LeftToRight )
									{
										// 左→右
										pv = 0.5f + px * 0.5f ;
									}
									else
									if( m_TransitionType == TransitionType.RightToLeft )
									{
										// 右→左
										pv = 0.5f - px * 0.5f ;
									}
									else
									if( m_TransitionType == TransitionType.TopToBottom )
									{
										// 上→下
										pv = 0.5f + py * 0.5f ;
									}
									else
									if( m_TransitionType == TransitionType.BottomToTop )
									{
										// 下→上
										pv = 0.5f - py * 0.5f ;
									}
