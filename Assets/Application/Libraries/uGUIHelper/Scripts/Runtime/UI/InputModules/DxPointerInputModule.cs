...
namespace uGUIHelper
{
	...
	/// </summary>
	public abstract class DxPointerInputModule : DxBaseInputModule
	{
		/// <summary>
		...
		protected bool GetPointerData( int id, Vector2 basePosition, out PointerEventData data, bool create )
		{
		...
		/// <returns></returns>
		protected PointerEventData GetTouchPointerEventData( Touch input, out bool pressed, out bool released, out bool pressing )
		{
			PointerEventData pointerData ;
			var created = GetPointerData( input.fingerId, input.position, out pointerData, true ) ;

			pointerData.Reset() ;

			pressed = created || ( input.phase == TouchPhase.Began ) ;
			released = ( input.phase == TouchPhase.Canceled ) || ( input.phase == TouchPhase.Ended ) ;
			pressing = pressed || ( input.phase == TouchPhase.Stationary ) || ( input.phase == TouchPhase.Moved ) ;
		...
		// ボタンの押下状態を取得する
		protected bool IsPressingForMouseButton( int buttonId )
		{
		...
		protected class MouseState
		{
			...
			public void SetButtonState( PointerEventData.InputButton button, bool isPressing, PointerEventData.FramePressState stateForMouseButton, PointerEventData data )
			{
				var toModify = GetButtonState( button ) ;
				toModify.eventData.IsPressing	= isPressing ;	// 拡張
		...
		protected virtual MouseState GetMousePointerEventData( int id )
		{
			...
			var created = GetPointerData( kMouseLeftId, Input.mousePosition, out leftData, true ) ;
			...
			GetPointerData( kMouseRightId, Input.mousePosition, out rightData, true ) ;
			rightData.Reset() ;
			...
			GetPointerData( kMouseMiddleId, Input.mousePosition, out middleData, true ) ;
			middleData.Reset() ;

			CopyFromTo( leftData, middleData ) ;
			middleData.button = PointerEventData.InputButton.Middle ;

			m_MouseState.SetButtonState( PointerEventData.InputButton.Left,		IsPressingForMouseButton( 0 ),	StateForMouseButton( 0 ), leftData   ) ;
			m_MouseState.SetButtonState( PointerEventData.InputButton.Right,	IsPressingForMouseButton( 1 ),	StateForMouseButton( 1 ), rightData  ) ;
			m_MouseState.SetButtonState( PointerEventData.InputButton.Middle,	IsPressingForMouseButton( 2 ),	StateForMouseButton( 2 ), middleData ) ;
		...
		protected PointerEventData GetLastPointerEventData( int id )
		{
			...
			GetPointerData( id, Input.mousePosition, out data, false ) ;
			return data ;
		}
