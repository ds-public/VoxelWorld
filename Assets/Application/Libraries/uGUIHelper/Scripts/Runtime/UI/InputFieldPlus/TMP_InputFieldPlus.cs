...
//					case RuntimePlatform.Stadia:
		...
		public virtual void OnUpdateSelected(BaseEventData eventData)
		{
			...
			while (Event.PopEvent(m_ProcessingEvent))
			{
				...
				switch (m_ProcessingEvent.rawType)
				{
					...
					case EventType.KeyDown:
						...
						// Windows:ＩＭＥ入力完了時に２重に入力されるバグに対処する(フォーカスをロストしたケース)
			...
			if( consumedEvent )
			{
				...
				if( string.IsNullOrEmpty( m_CompositionString ) == false && m_FixedText != m_Text )
				{
		...
		private void Delete()
		{
			...
			if( m_isRichTextEditingAllowed || m_isSelectAll )
			{
				...
				if( m_StringPosition <  m_StringSelectPosition )
				{
					if( text.Length >  0 && m_StringPosition >= 0 && m_StringPosition <  text.Length && ( m_StringSelectPosition - m_StringPosition ) >  0 )
					{
						int length = m_StringSelectPosition - m_StringPosition ;
						if( length >  ( text.Length - m_StringPosition ) )
						{
							length  = ( text.Length - m_StringPosition ) ;
						}
						if( length >  0 )
						{
							m_Text = text.Remove( m_StringPosition, length ) ;
						}
						else
						{
							m_Text = text ;
				...
				{
					if( text.Length >  0 && m_StringSelectPosition >= 0 && m_StringSelectPosition <  text.Length && ( m_StringPosition - m_StringSelectPosition ) >  0 )
					{
						int length = m_StringPosition - m_StringSelectPosition ;
						if( length >  ( text.Length - m_StringSelectPosition ) )
						{
							length  = ( text.Length - m_StringSelectPosition ) ;
						}
						if( length >  0 )
						{
							m_Text = text.Remove( m_StringSelectPosition, length ) ;
						}
						else
						{
							m_Text = text ;
				...
				if (m_isRichTextEditingAllowed || m_isSelectAll)
				{
					...
					if( m_StringPosition <  m_StringSelectPosition )
					{
						if( text.Length >  0 && m_StringPosition >= 0 && m_StringPosition <  text.Length && ( m_StringSelectPosition - m_StringPosition ) >  0 )
						{
							...
							if( length >  0 )
							{
								validateText = text.Remove( m_StringPosition, length ) ;
							}
							else
							{
								validateText = text ;
					...
					else
					{
						if( text.Length >  0 && m_StringSelectPosition >= 0 && m_StringSelectPosition <  text.Length && ( m_StringPosition - m_StringSelectPosition ) >  0 )
						{
							...
							if( length >  0 )
							{
								validateText = text.Remove( m_StringSelectPosition, length ) ;
							}
							else
							{
								validateText = text ;
		...
		private void Insert(char c)
		{
			...
			if( m_StringPosition <  0 )
			{
				m_StringPosition  = 0 ;
			}
			else
			if( m_StringPosition >= text.Length )
			{
				m_StringPosition  = text.Length ;
			}
		...
		protected void SendOnEndEdit()
		{
			...
			if( m_TextComponent != null )
			{
				m_TextComponent.rectTransform.anchoredPosition	= new Vector2( 0, 0 ) ;
				m_TextComponent.rectTransform.anchorMin			= new Vector2( 0, 0 ) ;
				m_TextComponent.rectTransform.anchorMax			= new Vector2( 1, 1 ) ;

				if( caretRectTrans != null )
				{
					caretRectTrans.anchoredPosition	= new Vector2( 0, 0 ) ;
					caretRectTrans.anchorMin		= new Vector2( 0, 0 ) ;
					caretRectTrans.anchorMax		= new Vector2( 1, 1 ) ;
				}
			}
			//==========================================================
