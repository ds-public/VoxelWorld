...
namespace UnityEngine.UI
{
		...
		ILayoutElement
	{
		...
		protected int caretPositionInternal
		{
			get
			{
				if( string.IsNullOrEmpty( m_CompositionString ) == true )
				{
					return m_CaretPosition ;
				}
				return m_CaretPosition + m_CompositionString.Length ;
			}
			set
			{
		...
		protected int caretSelectPositionInternal
		{
			get
			{
				if( string.IsNullOrEmpty( m_CompositionString ) == true )
				{
					return m_CaretSelectPosition ;
				}
				return m_CaretSelectPosition + m_CompositionString.Length ;
			}
			set
			{
...
//			return m_TextComponent.font.HasCharacter(c);
			// Unity2018.2 以降のバグ対応(存在するフォントまで存在しないと返されるためフォントの存在チェックは省く)
			return true ;
			//----------------------------------
		}
		...
		private void Insert(char c)
		{
			...
			// 全角入力を決定した際にキャレットの位置決定が間違っていておかしな位置に２文字目以降の全角文字が挿入されてしまう問題を修正する
//			caretSelectPositionInternal = caretPositionInternal += replaceString.Length;
			m_CaretPosition += replaceString.Length ;
		...
		protected void UpdateLabel()
		{
			if (m_TextComponent != null && m_TextComponent.font != null && !m_PreventFontCallback)
			{
...
#if UNITY_EDITOR || UNITY_STANDALONE
				if( string.IsNullOrEmpty( m_CompositionString ) == false && m_AllowInput == true )
				{
					fullText = text.Substring( 0, m_CaretPosition ) + m_CompositionString + text.Substring( m_CaretPosition ) ;

					if( m_TextColorModifier != null )
					{
						modifierIndex = m_CaretPosition ;
						int i, l = m_CompositionString.Length ;
						modifierColor = new Color[ l ] ;
						for( i  = 0 ; i <  l ; i ++ )
						{
							modifierColor[ i ] = m_ImeColor ;
						}
					}
				}
				else
...
#else
				if( string.IsNullOrEmpty( compositionString ) == false && m_AllowInput == true )
				{
				...
				if( m_TextColorModifier != null )
				{
					if( m_AllowInput == true )
					{
						m_TextColorModifier.index = modifierIndex - m_DrawStart ;
						m_TextColorModifier.color = modifierColor ;
					}
					else
					{
						m_TextColorModifier.index = 0 ;
						m_TextColorModifier.color = null ;
					}
				}
				//---------------------------------
				MarkGeometryAsDirty() ;
		...
		private void SetDrawRangeToContainCaretPosition(int caretPos)
		{
			...
			else
			{
				...
				if (caretPos > m_DrawEnd || (caretPos == m_DrawEnd && m_DrawStart > 0))
				{
					...
					{
						if( m_DrawStart >= characters.Count )
						{
							Debug.LogWarning( "Over : " + m_DrawStart + " / " + characters.Count ) ;
						}
