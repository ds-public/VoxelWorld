...
namespace InputHelper
{
	...
	//		gameObject.hideFlags = HideFlags.HideInHierarchy ;
		...
		private InputTypes m_InputType = InputTypes.Pointer ;	// デフォルトはタッチモード
...
//						Debug.LogWarning( "タッチモードに移行する" ) ;
	...
	public class GamePad
	{
		...
		/// <returns></returns>
		public static bool SetProfile( int number, Profile profile )
		{
			if( number <  0 || number >= m_Profile.Length )
			{
				return false ;
...
//			},
		...
		public static void Update()
		{
			...
			int[] playerNumbers  = {  0,  1,  2,  3, -1 } ;
			int[] profileNumbers = {  0,  1,  2,  3, -1 } ;
			...
			for( i  = 0 ; i <  l ; i ++ )
			{
				...
				button = GetButtonAll( playerNumbers[ i ], profileNumbers[ i ] ) ;

				m = buttonNumbers.Length ;
				for( j  = 0 ; j <  m ; j ++ )
				{
					m_ButtonHoldStates[ i, j ] = false ;
					m_ButtonOnceStates[ i, j ] = false ;

					k = m_ButtonNumberIndex[ buttonNumbers[ j ] ] ;
						
					if( ( button & buttonNumbers[ j ] ) != 0 )
					{
				...
				// Axis
				m = axisNumbers.Length ;
				for( j  = 0 ; j <  m ; j ++ )
				{
					axis = GetAxis( axisNumbers[ j ], playerNumbers[ i ], profileNumbers[ i ] ) ;
